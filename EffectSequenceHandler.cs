...
namespace Game.GamePlay
{
    public class EffectSequenceHandler : MonoBehaviour
    {
        ...
        private void RegEvent()
        {
            ...
            _gamePassRewardSubscription = _eventService.Observe<RewardClaimedArgs, GameEvent>(GameEvent.OnRewardClaimByGamePass)
                .Subscribe(args => PlayGamePassRewardEffectAsync(args).Forget());
        }

        private async UniTaskVoid PlayGamePassRewardEffectAsync(RewardClaimedArgs args)
        {
            if (args == null || args.Rewards == null || args.Rewards.Count == 0) return;
    
            RectTransform parentRect = _parentTransform as RectTransform;
            if (parentRect == null) return;

            Vector2 spawnPos = Vector2.zero;
            if (args.SpawnTransform != null)
            {
                spawnPos = parentRect.InverseTransformPoint(args.SpawnTransform.position);
            }

            EffectBurstConfig runtimeConfig = Instantiate(rewardPool.Config);

            var fadeCommand = new UIRewardBurstAndFadeCommand(
                rewardPool, 
                runtimeConfig,
                _parentTransform,
                spawnPos,
                args.Rewards,
                onRewardCompleted: (resKey) =>
                {
                  
                });

            await _effectSequenceService.PlayAsync(fadeCommand);
        }

...
namespace Game.DependencyInjection
{
    public class ProjectScope: LifetimeScope
    {
        ...
        {
            new LocalizationInstaller(GetConfig<LocalizationData>());
            new TimeProviderInstaller().Install(builder);
            ...
            new TransitionScreenInstaller(transitionScreen).Install(builder);
            ...
            new ProfileInstaller(GetConfig<ProfileDatabase>()).Install(builder);
            new GamePassInstaller(GetConfig<PassDatabase>()).Install(builder);
            ...
            builder.Register<ServiceOrderedInitialiser>(Lifetime.Singleton)
                .WithParameter(container => new List<List<IServiceInitialisable>>
                {
                    ...
                    new()
                    {
                        ...
                        new ActionInitialisable(token => container.Resolve<SaveSystem>().InitializeAsync(token)),
                    },
                    // --- LAYER 1
                    new()
                    {
                        new ActionInitialisable(token => container.Resolve<TransitionScreen>().InitializeAsync(token)),
                        container.Resolve<MockIapBridge>(),
                        new ActionInitialisable(token => container.Resolve<AudioManager>().InitializeAsync(token)),
                    ...
                    new()
                    {
                        new ActionInitialisable(token => container.Resolve<ResourceManager>().InitializeAsync(token)),
                        new ActionInitialisable(token => container.Resolve<PopupManager>().InitializeAsync(token)),
                    },

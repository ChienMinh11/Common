...
namespace ChieChie.Resource
{
    public class ResourceManager : IResourceService, IDisposable
    {
        ...
        private readonly List<ResourcePresenter<long>> _activePresenters = new();
        ...
        public event Action<string> OnResourcePendingUpdatesProcessed;
        ...
        public void RegisterView(string resourceKey, IResourceView view)
        {
            if (!IsInitialized || view == null) return;

            UnregisterView(view);
    
            var presenter = new ResourcePresenter<long>(_longModel, view, resourceKey, new LongConverter(), this);

            _activePresenters.Add(presenter);
            presenter.OnPendingUpdatesProcessed += HandlePresenterPendingUpdatesProcessed;

            var regenPresenter = new ResourceRegenPresenter(view, resourceKey, this);
            regenPresenter.Initialize();
            _activeRegenPresenters[view] = regenPresenter;
        }

        public void UnregisterView(IResourceView view)
        {
            if (view == null) return;
            ...
            for (int i = _activePresenters.Count - 1; i >= 0; i--)
            {
                ...
                if (presenter.View != view) continue;
            ...
            if (_activeRegenPresenters.TryGetValue(view, out var regenPresenter))
            {
                regenPresenter.Dispose();
                _activeRegenPresenters.Remove(view);
            }
        }
        ...
        public bool IsInfiniteDisplayDelayed(string resourceKey)
        {
            ...
            for (int i = 0; i < _activePresenters.Count; i++)
            {
                ...
                {
                    if (presenter.IsInfiniteUpdateDelayed)
                    {
                        return true;
                    }
                }
        ...
        public void ProcessPendingUpdate(string resourceKey, long amountIncrement = 0)
        {
            ...
            for (int i = 0; i < _activePresenters.Count; i++)
            {
                ...
                {
                    _activePresenters[i].ProcessPendingUpdates(amountIncrement);
                }
            }
        }
        ...
        private void HandlePresenterPendingUpdatesProcessed(string resourceKey)
        {
            OnResourcePendingUpdatesProcessed?.Invoke(resourceKey);
        }
        ...
        public void Dispose()
        {
            ...
            foreach (var regenPresenter in _activeRegenPresenters.Values)
            {
                regenPresenter?.Dispose();
            }
            _activeRegenPresenters.Clear();

    ...
    public class PopupController : IPopupService, IPopupQueueService, IDisposable
    {
        ...
        public async UniTask<bool> ShowPopup(string popupNameId, string message = "", bool closeAndRestore = false)
        {
            ...
            if (_popups.TryGetValue(hashKey, out IPopup cachedPopup) && cachedPopup != null)
            {
                ...
                return await ShowPopupByHashInternal(cachedPopup, hashKey, message, closeAndRestore);
            }
            if (_loadingPopupHashes.Contains(hashKey))
            {
                return false; 
            ...
            bool isShowSuccess = await ShowPopupByHashInternal(targetPopup, hashKey, message, closeAndRestore);
        ...
        private async UniTask<bool> ShowPopupByHashInternal(IPopup targetPopup, int hashKey, string message = "", bool closeAndRestore = false)
        {

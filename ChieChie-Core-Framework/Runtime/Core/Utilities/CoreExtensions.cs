...
namespace ChieChie.Core
{
    public static class CoreExtensions
    {
        ...
        public static int Sum<T>(this T[] array, Func<T, int> sumFun)
        {
            ...
            for (var i = 0; i < array.Length; i++) sum += sumFun(array[i]);
        ...
        public static int Sum<T>(this List<T> list, Func<T, int> sumFun)
        {
            ...
            for (var i = 0; i < list.Count; i++) sum += sumFun(list[i]);
        ...
        {
            if (array.IsInRange(index)) return array[index];
        ...
        {
            if (list.IsInRange(index)) return list[index];
        ...
        public static T FindRandomOrder<T>(this List<T> list, Func<T, bool> action)
        {
            ...
            while (tabsIndices.Count > 0)
            {
                ...
                if (action(list[index])) return list[index];
            }
        ...
        public static T FindRandomOrder<T>(this T[] array, Func<T, bool> action)
        {
            ...
            while (tabsIndices.Count > 0)
            {
                ...
                if (action(array[index])) return array[index];
            }
        ...
        public static string ObjectToString(this object parentObject, int maxDepth = 3)
        {
            ...
            stringBuilder.AppendLine("<b>" + parentObject + "</b>");
        ...
        private static string GetFields(int depth, int maxDepth, string space, object parentObject)
        {
            ...
            var fieldInfos =
                parentObjectType.GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            ...
            for (var i = 0; i < fieldInfos.Length; i++)
            {
                ...
                if (fieldType.IsClass && fieldType != typeof(string))
                {
                    if (fieldType.IsArray)
                    {
                        ...
                        if (elementType != null)
                        {
                            var arrayFieldInfos = elementType.GetFields(BindingFlags.Instance | BindingFlags.NonPublic |
                                                                        BindingFlags.Public);

                            var list = (IList)value;
                            if (list != null)
                            {
                                stringBuilder.AppendLine(space + fieldName + " - " + fieldType);
